...
(function($) {
    $.fn.extend({
        ...
        jdigiclock: function(options) {
            ...
            var defaults = {
                clockImagesPath: basePath +'static/css/images/forecast/clock/',
                weatherImagesPath: basePath +'static/css/images/forecast/weather/',
                lang: 'en',
                am_pm: false,
                weatherLocationCode: 'ASI|CN|CH002|BEIJING',
                weatherMetric: 'C',
                weatherUpdate: 1,
                proxyType: 'php'
            ...
            return this.each(function() {
                ...
                html    += "<p id='left_arrow'><img src='"+basePath+"static/css/images/forecast/icon_left.png' /></p>";
                html    += "<p id='right_arrow'><img src='"+basePath+"static/css/images/forecast/icon_right.png' /></p>";
    ...
    $.fn.getTime = function(el) {
        ...
        // date
        el.currDate = el.lang.monthNames[now.getMonth()] + now.getDate()+"日&nbsp;<br/>" + el.lang.dayNames[now.getDay()] ;
        // time update
    ...
    $.fn.getWeather = function(el) {
        ...
        var proxy = '';
        var uri = basePath + "static/css/images/forecast/weather";
        var imgArr = [];
        var imgName = [];
		...
		success : function(json) {
			//另外处理得到的
			var data = json.jsonData[0].weatherinfo;

            el.find('#weather .loading, #forecast_container .loading').hide(); 

			//定义：当前气温,   最高气温
            var curr_temp = '<p class="temp">' + data.temp1 + '</p>';
            //气象图片
            //定义图片路径
            //处理天气图像的局部路径
            var imgUrl = "";
            function getImgPath(str, img){
            	if(parseInt(img)<=9) imgUrl= str +"0" + img;
            	else imgUrl = str + img;
            }
            //根据白天还是晚上分别处理
            var hour = new Date().getHours();
            if(hour>6&&hour<=18){
            	getImgPath("d", data.img1);
            }else{
            	if(data.img2=="99") getImgPath("n", data.img1);
            	getImgPath("n", data.img2);
            }
            
            el.find('#weather').css({'background':'url(' + el.weatherImagesPath + imgUrl + '.gif) 48% 50% no-repeat'
            });
			//定义：城市 + 气象术语
            var weather = '<div id="local"><p class="city">海淀</p><p style="font-size:12px;margin:4px 0;">' + data.weather1 + '</p><p style="font-size:12px;">'+ data.wind1+'</p></div>';
			//      日期，当前气温
            weather += '<div id="temp"><p id="date" style="margin:3px 0;">' + el.currDate + '</p><span style="margin-top:8px;">'+curr_temp + '<span></div>';
            ...
            el.find('#forecast_container').append('<div id="update"><img src='+ basePath+'"images/refresh_01.png" alt="reload" title="reload" id="reload" />' + el.timeUpdate + '</div>');
