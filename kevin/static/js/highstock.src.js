...
Scroller.prototype = {
	...
	drawHandle: function (x, index) {
		...
		if (!scroller.rendered) {
			...
			// the rectangle  //modified:前(-4.5, 0, 9, 16, 3, 1)
			tempElem = renderer.rect(-4.5, -6.5, 9, 30, 3, 1)
				.attr(attr)
				.add(handles[index]);
...
RangeSelector.prototype = {
	...
	setInputValue: function (input, time) {
		var rangeSelector = this,
			chart = rangeSelector.chart,
			options = chart.options.rangeSelector,
			format = input.hasFocus ? options.inputEditDateFormat || '%Y-%m-%d' : options.inputDateFormat || '%Y-%m-%d';//改过,对应从。。。到。。。的时间
